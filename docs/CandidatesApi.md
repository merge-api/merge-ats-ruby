...
> <Array(nil, Integer, Hash)> candidates_ignore_create_with_http_info(model_id, ignore_common_model_request)

```ruby
begin
  
  data, status_code, headers = api_instance.candidates_ignore_create_with_http_info(model_id, ignore_common_model_request)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => nil
rescue MergeATSClient::ApiError => e
