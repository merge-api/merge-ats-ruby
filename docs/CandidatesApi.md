...
> <Candidate> candidates_create(x_account_token, opts)
...
begin
  ...
  result = api_instance.candidates_create(x_account_token, opts)
  p result
...
> <Array(<Candidate>, Integer, Hash)> candidates_create_with_http_info(x_account_token, opts)

```ruby
begin
  
  data, status_code, headers = api_instance.candidates_create_with_http_info(x_account_token, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <Candidate>
rescue MergeATSClient::ApiError => e
