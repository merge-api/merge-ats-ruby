...
> <Attachment> attachments_create(x_account_token, opts)
...
begin
  ...
  result = api_instance.attachments_create(x_account_token, opts)
  p result
...
> <Array(<Attachment>, Integer, Hash)> attachments_create_with_http_info(x_account_token, opts)

```ruby
begin
  
  data, status_code, headers = api_instance.attachments_create_with_http_info(x_account_token, opts)
  p status_code # => 2xx
  p headers # => { ... }
  p data # => <Attachment>
rescue MergeATSClient::ApiError => e
